...
**Phase 9.3: HTTPS/HSTS/CSP Proxy & Deployment Posture** - **COMPLETED** (closes 5E.3 known-limitation #6 per the documented "TLS at proxy" architecture: no in-app TLS, zero certificate material in-repo. App side: opt-in TRUST_PROXY (default off — no IP-spoofing regression), production-only HSTS (max-age=31536000, no preload/includeSubDomains) + strict JSON-API CSP (`default-src 'none'; frame-ancestors 'none'`) beside the 5E.3 headers; dev emits neither (verified). Deployment side: complete reference nginx config (TLS 1.2/1.3, 301 redirect, matching HSTS, WebSocket upgrade, minimum-viable frontend CSP audited against the actual bundle), security-tradeoff ledger (code vs proxy vs deploy-time), 6-step deploy checklist. New 38-assertion posture suite (dev/prod header logic, cookie flags, static proxy-config verification); full 16-suite battery green at 477 assertions. All three DOCUMENTED Phase 9 candidates (9.1, 9.2, 9.3) now COMPLETE — remaining Phase 9 items are optional candidates only: email/push channels, HTTP caching/ETags — not started)

**Next: optional Phase 9 candidates** (email/push channels; HTTP caching/ETags — per roadmap notes, not required to close Phase 9)

**Phase 9.1: Refresh Tokens & Token Revocation** - **COMPLETED** (closes 5E.3 known-limitation #1: bearer tokens used to stay valid ≤15 min after logout. New in-memory session store (single-node pattern): 7-day session families, single-use rotating refresh tokens (sha256-hashed, httpOnly cookie transport, never persisted client-side), replay-theft detection (reuse revokes the family + live access tokens), logout revokes server-side; access tokens carry `sid` checked O(1) in the shared REST+socket verifier; pre-9.1 tokens accepted for rolling restarts; NEW POST /api/auth/refresh; frontend silent-refresh + retry on 401-expired (single-flight) and socket reconnect-once; 5E.1 posture test flipped to the secure behavior as documented; new 28-assertion refresh-auth suite; full battery green; `REFRESH_TOKEN_TTL_MS` documented)

**Phase 9.2: Broader REST Rate Limiting** - **COMPLETED** (closes 5E.3 known-limitation #5: new restLimiter — established in-memory fixed-window pattern — mounted after public routes (health/auth exempt) and before the protected routers; 600 req/min/IP default via REST_RATE_LIMIT/REST_RATE_WINDOW_MS; safe 429 + Retry-After; loginLimiter/joinLimiter preserved; battery proven unthrottled at defaults; rest-limit suite 19/19 default + 18/18 knobbed)

**Phase 8.4: Page-Content Headers, Empty States & Phase 8 Wrap-Up** - **COMPLETED** (audit: all 10 content headers verified consistent and left untouched; 7 real fixes: error/not-member back buttons to the 44px+ring standard on 4 pages, Groups rich empty state with working Create Group CTA, MyTasks empty-state CTAs to 44px+ring, MyTasks decorative SVG aria-hidden, GroupDetail defensive "No members yet." row, TaskDetail/GroupDetail long-title break-words; frontend-only; build + full battery green). **PHASE 8 COMPLETE**
