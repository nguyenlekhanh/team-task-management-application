...
**Phase 9.2: Broader REST Rate Limiting** - **COMPLETED** (closes 5E.3 known-limitation #5: no request-rate limiting on non-auth REST endpoints. New `restLimiter` middleware — the established in-memory fixed-window pattern applied to request counting — mounted in routes/index.js AFTER public routes (health/auth exempt by construction) and BEFORE the protected routers (users/groups/notifications/tasks/messages, every method; check runs before authenticate/DB). Default 600 req/min/IP via `REST_RATE_LIMIT`/`REST_RATE_WINDOW_MS`; 429 safe envelope + Retry-After; req.ip identification, no trust-proxy (documented as a reverse-proxy deployment step); loginLimiter/joinLimiter semantics preserved; battery proven unthrottled at defaults; new 19/18-assertion suite (default + knobbed sections); full 15-suite battery green at 439 assertions. Phase 9: IN PROGRESS — 9.3 HTTPS/HSTS/CSP deployment posture remains)

**Next: Phase 9.3** (HTTPS/HSTS/CSP proxy/deployment posture — next documented candidate, not started)
