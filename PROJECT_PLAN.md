...
**Phase 9.1: Refresh Tokens & Token Revocation** - **COMPLETED** (closes 5E.3 known-limitation #1: bearer tokens used to stay valid ≤15 min after logout. New in-memory session store (single-node pattern): 7-day session families, single-use rotating refresh tokens (sha256-hashed, httpOnly cookie transport, never persisted client-side), replay-theft detection (reuse revokes the family + live access tokens), logout revokes server-side; access tokens carry `sid` checked O(1) in the shared REST+socket verifier; pre-9.1 tokens accepted for rolling restarts; NEW POST /api/auth/refresh; frontend silent-refresh + retry on 401-expired (single-flight) and socket reconnect-once; 5E.1 posture test flipped to the secure behavior as documented; new 28-assertion refresh-auth suite; full 14-suite battery green at 420 assertions; `REFRESH_TOKEN_TTL_MS` documented. Phase 9: IN PROGRESS — 9.2 broader REST rate limiting, 9.3 HTTPS/HSTS/CSP remain)

**Next: Phase 9.2** (broader REST rate limiting — next documented candidate, not started)
